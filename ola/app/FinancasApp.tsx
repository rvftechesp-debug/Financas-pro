            ...
            <Card className="bg-white/[0.03] border-white/[0.07]">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-bold text-[#ccc] flex items-center gap-2">
                  ...
                  Lançamentos ({finance.filtered.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                {finance.filtered.length === 0 && (
                  <p className="text-[#666] text-[13px]">Nenhum lançamento neste mês.</p>
                )}
                <div className="space-y-0">
                  {[...finance.filtered].sort((a, b) => +new Date(b.date) - +new Date(a.date)).map((e: Expense) => {
                    const cat = CATEGORIES.find(c => c.name === e.category);
                    return (
                      <div key={e.id} className="flex items-center gap-3 py-3.5 border-b border-white/[0.03] last:border-b-0">
            ...
            <Card className="bg-white/[0.03] border-white/[0.07] overflow-hidden">
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                  ...
                  />
                  <select
                    className="bg-white/5 border border-white/10 rounded-xl text-[#f0f0f0] px-3.5 py-2.5 text-sm outline-none focus:border-emerald-500 w-full cursor-pointer transition-colors"
                ...
                <button
                  onClick={() => {
                    ...
                    finance.addIncome(valorReceita);
                    setValorReceita("");
